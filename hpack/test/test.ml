...
let parse_file file =
  ...
  let description = match Json.(json |> member "description" |> to_string_option) with
  | Some x -> x
  | None -> file
  in
  let cases = List.map begin fun case ->
...
let encode cases =
  ...
  List.mapi begin fun seq (_nosize, _nowire, headers) ->
...
let encode_file fixtures_dir (story, file) =
  ...
  in
  let channel = open_out (Filename.(concat fixtures_dir (concat "ocaml-hpack" story))) in
...
let header_testable = (module struct
  type t = header

  let pp formatter {name; value; _} =
    Fmt.pf
      formatter
      "%s: %s" name value
...
let decode cases =
  ...
  (* Note:
       Encoders and decoders are stateful. To check rountripping we need to
       use 2 decoders. Here's why:
         We initially decode the headers we parsed from JSON and assert that
         they're the same as the JSON we got.

         We then want to:
           1. Encode the resulting headers
           2. Decode them again
           3. Check they are indeed the same

         The reason why we need 2 decoders is because of `1.` above. Since
         compression / decompression state is stateful, and we don't have
         access to the first encoder (which may have state about indexed
         fields that `decoder1` has computed in the meantime), using `decoder1`
         would produce wrong headers (given wrong expectations about indexed
         header fields).

         From then on, we can feel free to use the `encoder` / `decoder2` pair,
         as that effectively mimics the same "connection". In fact, encoding
         and decoding the same headers multiple times will make the compressed
  ...
  List.iter (fun (size, wire, headers) ->
    ...
    Alcotest.(check int) "same length" (List.length headers) (List.length decoded_headers);
...
let gen_suites fixtures =
  let gen_suite filename =
    ...
    test_case_name, `Slow, fun () ->
      decode fixture
    in
  List.map (fun (suite_name, files) ->
...
let () =
  ...
  in
  begin try Unix.mkdir (Filename.concat fixtures_dir "ocaml-hpack") 0o755
