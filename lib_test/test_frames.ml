...
module P = struct
  ...
  let parse_fn wire success_handler error_handler =
    ...
    let _read =
      Reader.read_with_more reader wire_bs ~off:0 ~len:(Bigstringaf.length wire_bs) Incomplete
    in
    reader
  ...
  let parse_success wire handler =
    ...
    | `Read -> ()
    | `Close
    | `Error _ -> Alcotest.(fail "Expected to have sucessfully parsed frame.")
  ...
  let parse_error wire handler =
    ...
    match Reader.next reader with
    | `Read -> ()
    | `Close -> Alcotest.(fail "Expected to have thrown an error parsing frame.")
    | `Error e -> handler e
...
let frame_testable = (module struct
  type t = Frame.t
  ...
  let frame_payload_to_json frame_type payload =
    let others = match payload with
    | Frame.Data data -> ["data", `String (bs_to_string data)]
    | Headers (priority, fragment) ->
      ("header_block_fragment", `String (bs_to_string fragment)) :: (priority_to_yojson priority)
    | Priority priority -> priority_to_yojson (Some priority)
  ...
  let equal = (=)
end : Alcotest.TESTABLE with type t = Frame.t)

let priority_of_json json =
  let stream_dependency = Json.(json |> member "stream_dependency" |> to_int_option) in
...
let priority_of_json_exn json =
  ...
  | Some p -> p
  | None -> raise (Invalid_argument "priority_of_json")
...
let frame_payload_of_json frame_type json =
  ...
  | Headers ->
    ...
    let fragment = Json.(json |> member "header_block_fragment" |> to_string |> bs_of_string) in
    Headers (priority, fragment)
  | Priority ->
    Priority (priority_of_json_exn json)
  | RSTStream ->
    let error_code = Json.(json |> member "error_code" |> to_int |> Int32.of_int)
    in
  ...
  | PushPromise ->
    let fragment = Json.(json |> member "header_block_fragment" |> to_string |> bs_of_string) in
    let promised_stream_id = Json.(json |> member "promised_stream_id" |> to_int |> Int32.of_int) in
    PushPromise (promised_stream_id, fragment)
  | Ping ->
    let opaque_data = Json.(json |> member "opaque_data" |> to_string |> bs_of_string) in
    Ping opaque_data
  | GoAway ->
    let error_code = Json.(json |> member "error_code" |> to_int |> Int32.of_int) in
    let last_stream_id = Json.(json |> member "last_stream_id" |> to_int |> Int32.of_int) in
    let debug_data = Json.(json |> member "additional_debug_data" |> to_string |> bs_of_string) in
    GoAway (last_stream_id, Error.parse error_code, debug_data)
  | WindowUpdate ->
    let window_size_increment = Json.(json |> member "window_size_increment" |> to_int) in
...
let success_suites =
  let gen_suite ~frame_type filename =
    ...
    Printf.sprintf "%s: %s" filename test_case_name, `Quick, fun () ->
    let expected_frame_json = Json.(member "frame" fixture) in
    let frame_payload_json = Json.(expected_frame_json |> member "frame_payload")
    in
    let frame_type = frame_type_of_string frame_type in
    let expected_frame =
      { Frame
      . frame_header =
        { payload_length = Json.(expected_frame_json |> member "length" |> to_int)
        ; flags = Json.(expected_frame_json |> member "flags" |> to_int)
        ; stream_id = Json.(expected_frame_json |> member "stream_identifier" |> to_int |> Int32.of_int)
        ; frame_type
        }
      ; frame_payload = frame_payload_of_json
          frame_type
          frame_payload_json
      }
    in
    ...
    P.parse_success wire (fun frame ->
      ...
      let padding = match Json.(frame_payload_json |> member "padding" |> to_string_option) with
      | Some padding ->
        Some Bigstringaf.(of_string ~off:0 ~len:(String.length padding) padding)
      | None -> None
      in
      let serialized_wire = P.serialize ?padding frame in
      Alcotest.(check string "Parse / Serialize roundtripping" wire serialized_wire))
  in
  ...
  let gen_suite filename =
    ...
    P.parse_error wire (function
      | Error.ConnectionError (e, _) | Error.StreamError (_, e) ->
        Alcotest.(check int32) "Expected Error Code" expected_error (Error.serialize e))
  in
  List.map (fun (_, files) ->
    "Error tests", List.map gen_suite files)
  error_fixtures

let () =
  Alcotest.run "lambda-runtime" (success_suites @ error_suites)
