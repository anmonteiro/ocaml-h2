...
module Client_connection_tests = struct
  ...
  let test_simple_get_request () =
    ...
    let response_handler _response _response_body =
      handler_called := true
    in
    let request_body =
      Client_connection.request t request
    ...
    | `Write iovecs ->
      ...
      | `Write iovecs ->
        ...
        Alcotest.(check int) "Next write operation is an empty DATA frame with the END_STREAM flag set"
          (Frame.FrameType.serialize Data)
          (Frame.(frame.frame_header.frame_type |>FrameType.serialize));
        Alcotest.(check bool) "Next write operation is an empty DATA frame with the END_STREAM flag set"
          true
