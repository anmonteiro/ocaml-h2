...
let single_get =
  [ "single GET"
    ...
    , Simulator.test_server
        ...
        ~input:   [(`Request (Request.create ~headers:(Headers.of_list ["a", "b"]) `GET "/")), `Empty]
...
let streaming_response =
  ...
  , Simulator.test_server
      ...
      ~input:   [ `Request (Request.create `GET "/"), `Empty ]
      ~output:  [ `Response (Response.create `OK), `Data ["Hello,"; " world!"] ]
  ...
  , Simulator.test_server
      ~handler: (fun reqd ->
        ...
        Body.flush body (fun () ->
          Body.close_writer body))
      ~input:   [ `Request (Request.create ~headers:Headers.(of_list ["connection", "close"]) `GET "/"), `Empty ]
      ~output:  [ `Response (Response.create `OK), `Data [] ]
  ]
